...
public:
  ...
  explicit DataDictionary( std::istream& stream ) throw( ConfigError );
  explicit DataDictionary( const std::string& url ) throw( ConfigError );
  ...
  message_order & getMessageOrder() const
  {
     // delayed initialization because of addField, that is called after constructor
     if (!m_messageOrder)
     {
        m_messageOrder.reset( new message_order( m_orderedFields ) );
     }
     return *m_messageOrder;
  }
                 ...
                 const DataDictionary*& pDataDictionary ) const
  {
    ...
    if ( i == m_groups.end() )
       return false;
    std::pair < int, DataDictionary* > const & pair = i->second;
    delim = pair.first;
...
private:
  ...
  mutable boost::shared_ptr<message_order> m_messageOrder;
